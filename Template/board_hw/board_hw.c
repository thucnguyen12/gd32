...
//   gpio_mode_set(GPIO_LCD_PWM_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_LCD_PWM_PIN);
    ...
    //spi gpio
    gpio_mode_set(GPIO_SPI0_SCK_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_SPI0_SCK_PIN);
    gpio_output_options_set(GPIO_SPI0_SCK_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_SPI0_SCK_PIN);
    
    gpio_mode_set(GPIO_SPI0_MISO_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_SPI0_MISO_PIN);
    gpio_output_options_set(GPIO_SPI0_MISO_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_SPI0_MISO_PIN);
    
    gpio_mode_set(GPIO_SPI0_MOSI_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_SPI0_MOSI_PIN);
    gpio_output_options_set(GPIO_SPI0_MOSI_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_SPI0_MOSI_PIN);
    
    //lora
    gpio_mode_set(GPIO_LORA_CS_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_LORA_CS_PIN);
    gpio_output_options_set(GPIO_LORA_CS_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, GPIO_LORA_CS_PIN);
    
    gpio_mode_set(GPIO_LORA_IO4_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, GPIO_LORA_IO4_PIN);
    ...
    usart_baudrate_set(RCU_USART0, 115200U);
    usart_receive_config(RCU_USART0, USART_RECEIVE_ENABLE);
    usart_transmit_config(RCU_USART0, USART_TRANSMIT_ENABLE);

    usart_enable(RCU_USART0);
    usart_interrupt_enable(RCU_USART0, USART_INT_RBNE);
	usart_interrupt_enable(RCU_USART0, USART_INT_TBE);	
    ...
    usart_baudrate_set(RCU_USART1, 115200U);
